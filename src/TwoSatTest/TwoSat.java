...
public final class TwoSat {
     ...
     */
    public static <T>  HashMap<Literal<T>, Boolean> isSatisfiable(List<Clause<T>> formula) {
        ...
        /* Compute condensation of the implication graph. */
        DirectedGraph<Integer> condensation = new DirectedGraph<Integer>();
        for (Integer i : scc.values()) {
        	condensation.addNode(i);
        }
        for (Literal<T> u : implications) {
        	for (Literal<T> v : implications.edgesFrom(u)) {
        		if (scc.get(u) != scc.get(v)) condensation.addEdge(scc.get(u), scc.get(v));
        	}
        }
        
        /* Compute complementary components. */
        HashMap<Integer, Integer> complementaryComponent = new HashMap<Integer, Integer>();
        for ( Literal<T> u: scc.keySet()) {
        	Literal<T> v = u.negation();
        	complementaryComponent.put(scc.get(u), scc.get(v));
        }
        
        /* Find topological order. */
        List<Integer> componentsInOrder = TopologicalSort.sort(condensation);
        
        /* Compute truth assignment for components. */
        HashMap<Integer, Boolean> truthAssignments = new HashMap<Integer, Boolean>();
        for (Integer i : componentsInOrder) {
        	if (!truthAssignments.containsKey(i)) {
        		truthAssignments.put(i, false);
        		truthAssignments.put(complementaryComponent.get(i), true);
        	}
        }
        
        /* Compute truth assignment for literals. */
        HashMap<Literal<T>, Boolean> result = new HashMap<Literal<T>, Boolean>();
        for (Literal<T> u : implications) {
        	result.put(u, truthAssignments.get(scc.get(u)));
        } 
        return result;
    }
} 